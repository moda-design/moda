...
`moda canvas read`, or `--revision`). `--screenshot PATH` captures every page the edit changed
(the response's `changed_page_ids`, auto-batched past the 3-per-call cap) right after the
commit — the same files `moda canvas screenshot -o PATH` would write, in one invocation. When
the edit changed no page (variable-only edits), it falls back to the current page. `--page`
only scopes the read snapshot and never steers the capture; a capture failure never changes
