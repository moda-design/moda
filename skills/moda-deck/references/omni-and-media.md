...
- `moda task start` is idempotent: an identical re-run replays the already-started task instead of spending again (the CLI says so when it detects the replay). A deliberate new attempt — e.g. after `task_failed` — takes `--fresh`.
