...
2. **Author each unit, one at a time, against the frozen spec.** Each unit gets the spec as written, its own page id, its content slice, and its acceptance-criteria lines — and gets built WITHOUT self-polishing. Serial is not a compromise here: writes to one canvas must serialize, and a parallel batch shares one revision pin and all but one member loses outright.
