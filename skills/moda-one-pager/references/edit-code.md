...
- `--screenshot PATH` captures the edited page (the `--page` target when given, else the default capture) right after the commit — the same files as `moda canvas screenshot -o PATH`, in one invocation. A capture failure never changes the edit's exit code.
