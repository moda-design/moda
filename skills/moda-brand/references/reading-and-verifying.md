...
- **A list result is a PAGE, not the universe.** Check `total`/`has_more` before telling the user how many things they have; fetch remaining pages (`--offset`, or `--all` — client-capped at 500 items) when the task needs the full set. On an older server that reports no total, say "at least N", never "N total".
