...
| `<connector>` | from="@NodeName:anchor", to="@Node:anchor", routing=straight\|elbowed\|curved | anchored line between shapes; do NOT place in flex containers |
...
- A local file becomes canvas-usable through `moda file upload PATH`: the result returns a durable `file_…` reference **plus a short ref usable directly as `src`** in markup (`<image src="img1">` / `image(ref)` fills). From-URL images: `moda file upload --from-url URL`.
...
- Line arrowheads: `startArrowShape` / `endArrowShape` = triangle | open | reverse-triangle | diamond | circle (omit for none); size with `pointerLength` and `pointerWidth`.
