...
- **No video inputs on canvases.** A canvas takes images and documents only — no video fill, no "video background". Video files exist on this surface only as inputs/outputs of the metered media video tools and of the page-animation exports (`export(format='mp4'|'gif', page=N)` — a page with no animation rejects typed `no_animation`). Never tell the user to put a video on a canvas; offer a still frame as a background image instead.
