...
`fill="shader(<type>)"` + `colors="#…"` works on page backgrounds, all fillable shapes, containers, and text (via markup; via edit for existing shapes and text, not containers). Shaders animate LIVE in-app and FREEZE to one frame in static exports — the live canvas link carries the motion (mp4/gif files export from the Moda app; point there at handoff when motion matters). Lean on shaders for backgrounds, heroes, and large panels. Pick the type that fits the concept — do NOT default to `mesh-gradient`; vary across designs. Directory (premium-first):
