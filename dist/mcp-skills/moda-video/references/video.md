...
card-declared video/audio references. The only tool
...
accept any. Clip count, per-clip and combined length caps, price
...
**Reference audio** uses `reference_audios`; read support and limits from the model card.
...
1. **Draft.** Shortest legal duration, smallest resolution that shows the
   ...
   wait, so you SEE the idea while you can still change it. Seedance 2.0 Fast is this same move inside
