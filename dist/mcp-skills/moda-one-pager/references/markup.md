...
| `<comment>` | x,y,node="@Name",thread="c1",page_id | non-node side effect: create/reply to comment threads; resolve via `canvas_edit` code `update('c1',{resolved:true})` |
...
- Variables: `$name` references reusable color values (see the `## Vars` legend in `canvas_read` output). Prefer variables for brand colors or any color used in multiple places.
